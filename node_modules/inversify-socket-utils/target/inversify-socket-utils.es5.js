...
const paramDecoratorFactory = (parameterType) => (name) => params(parameterType, name || "default");
const socketIO = paramDecoratorFactory(PARAMETER_TYPE.SOCKET_IO);
const socketID = paramDecoratorFactory(PARAMETER_TYPE.SOCKET_ID);
const connectedSocket = paramDecoratorFactory(PARAMETER_TYPE.CONNECTED_SOCKET);
const payload = paramDecoratorFactory(PARAMETER_TYPE.SOCKET_BODY);
const socketQueryParam = paramDecoratorFactory(PARAMETER_TYPE.SOCKET_QUERY_PARAM);
const socketRequest = paramDecoratorFactory(PARAMETER_TYPE.SOCKET_REQUEST);
const socketRooms = paramDecoratorFactory(PARAMETER_TYPE.SOCKET_ROOMS);
...
const getControllerMetadata = (constructor) => {
    const controllerMetadata = Reflect.getMetadata(METADATA_KEY.Controller, constructor);
    return controllerMetadata;
};
const getActionMetadata = (constructor) => {
    const actionMetadata = Reflect.getMetadata(METADATA_KEY.Action, constructor);
    return actionMetadata;
};
const getParameterMetadata = (constructor) => {
    const parameterMetadata = Reflect.getMetadata(METADATA_KEY.Parameter, constructor);
...
class InversifySocketServer {
    ...
    handleAction(socket, controller, action, parameters, payload) {
        ...
        this.container.getNamed(TYPE.Controller, controller.target.name)[action.key](...args);
    }
...
export { InversifySocketServer, TYPE, connectedSocket, controller, interfaces, onConnect, onDisconnect, onMessage, payload, socketID, socketIO, socketQueryParam, socketRequest, socketRooms };
//# sourceMappingURL=inversify-socket-utils.es5.js.map
